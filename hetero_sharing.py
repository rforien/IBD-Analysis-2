...
def ibd_sharing(coordinates, L, step, bin_lengths, G, sigma, population_sizes, pw_growth_rate=0, balance='isotropic',
    ...
    # if cumul, we return the expected nb of blocks longer than the specified lengths
    if cumul:
        a = 2
        b = 1
    else:
...
def plot_barycentric_coordinates(coordinates):
    L = np.sqrt(coordinates.shape[0]).astype(int)
    ...
    x = np.array(positions_x[:,0])
    y = np.array(positions_y[:,0])
...
def grid_fit(positions, sigma, coarse=.25, max_iterate=10):
    ...
    L = 2 * int(np.ceil((np.maximum(10 * np.max(sigma), 10 * step) + np.max(np.abs(positions), (0, 1))) / step))
    return step, L

def barycentric_coordinates(positions, L, step):
    '''
    ...
    alpha = scaled_positions - left  # to compute weight of points
